...
public class AnnotationDrivenBeanDefinitionParser implements BeanDefinitionParser {
    ...
    public BeanDefinition parse(Element element, ParserContext parserContext) {
        ...
        //访问令牌控制器
        RuntimeBeanReference accessTokenController = getAccessTokenController(element, source, parserContext);
        serviceRouterDef.getPropertyValues().add("oauthAccessTokenController", accessTokenController);
    ...
    private RuntimeBeanReference getAccessTokenController(Element element, Object source, ParserContext parserContext) {
    	 RuntimeBeanReference accessTokenControllerRef;
         if (element.hasAttribute("access-token-controller")) {
        	 accessTokenControllerRef = new RuntimeBeanReference(element.getAttribute("access-token-controller"));
         } else {
             RootBeanDefinition accessTokenControllerDef = new RootBeanDefinition(DefaultOauthAccessTokenController.class);
             accessTokenControllerDef.setSource(source);
             accessTokenControllerDef.setRole(BeanDefinition.ROLE_INFRASTRUCTURE);
             String accessTokenControllerName = parserContext.getReaderContext().registerWithGeneratedName(accessTokenControllerDef);
             parserContext.registerComponent(new BeanComponentDefinition(accessTokenControllerDef, accessTokenControllerName));
             accessTokenControllerRef = new RuntimeBeanReference(accessTokenControllerName);
         }
         return accessTokenControllerRef;
    }
