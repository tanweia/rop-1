...
public class AnnotationServletServiceRouter implements ServiceRouter {
    ...
    public void service(Object request, Object response) {
        ...
        logger.info("准备调用服务方法：{}({})",method,version);
    ...
    private class ServiceRunnable implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                //验证系统级参数的合法性
                logger.info("客户端{}:开始校验请求参数的合法性",ropRequestContext.getAppKey());
                logger.debug("请求参数信息：{}",new Gson().toJson(servletRequest.getParameterMap()));
                ...
                if (mainError != null) {
                    ...
                    logger.info("客户端{}:调用服务请求参数验证为不合法",ropRequestContext.getAppKey());
                    logger.debug("客户端{}:请求参数不合法,错误信息:{}",ropRequestContext.getAppKey(),
                    		new Gson().toJson(new ErrorResponse(mainError)));
                } else {
                    ...
                    if (mainError != null) {
                        ...
                        if (ropRequestContext.getRopResponse() == null) { //拦截器未生成response
                        	...
                        	logger.info("客户端{}:数据验证完毕，正式调用服务...",ropRequestContext.getAppKey());
                            ropRequestContext.setRopResponse(doService(ropRequest));

                            //输出响应前拦截
                            invokeBeforceResponseOfInterceptors(ropRequest);
                            logger.info("客户端{}:正式调用服务完毕",ropRequestContext.getAppKey());
                        }
                    }
                }
                //输出响应
    ...
    private void invokeBeforceServiceOfInterceptors(RopRequestContext ropRequestContext) {
        ...
        try {
            if (interceptors != null && interceptors.size() > 0) {
                for (Interceptor interceptor : interceptors) {
                    ...
                    if (ropRequestContext.getRopResponse() != null) {
                    	logger.debug("拦截器[{}]产生了一个RopResponse,阻止本次服务请求继续，服务将直接返回。",
            ...
            logger.error("在执行拦截器[{}]时发生异常.",tempInterceptor.getClass().getName(), e);
        }
    }
    ...
    private Object doService(RopRequest ropRequest) {
        ...
        if (context.getMethod() == null) {
            ...
            logger.info("客户端{}：调用的服务{}不存在",context.getAppKey(),context.getMethod());
            ...
            logger.info("客户端{}：调用的服务{}无效",context.getAppKey(),context.getMethod());
        } else {
            try {
                ropResponse = serviceMethodAdapter.invokeServiceMethod(ropRequest);
            } catch (Exception e) { //出错则招聘服务不可用的异常
                logger.error("客户端{}：调用{}时发生异常，异常信息为：{}",context.getAppKey(),
                		context.getMethod(),e.getMessage(),e);
