...
import com.rop.impl.DefaultOauthAccessTokenController;
...
public class DefaultSecurityManager implements SecurityManager {
    ...
    protected OauthAccessTokenController oauthAccessTokenController = new DefaultOauthAccessTokenController(); 
    ...
    public MainError validateSystemParameters(RopRequestContext context) {
        ...
        if (!appSecretManager.isValidAppKey(context.getAppKey())) {
        	logger.info("{}:无效的appkey",context.getAppKey());
        ...
        // 检查timestamp
        if (StringUtils.isEmpty(context.getTimestamp())) {
        	logger.info("{}:时间参数为空，不合法",context.getAppKey());
        ...
        // 检查access_token
        if(StringUtils.isEmpty(context.getAccessToken())){
        	logger.info("客户端{}:令牌为空，不合法",context.getAppKey());
        	return MainErrors.getError(MainErrorType.MISSING_ACCESS_TOKEN, context.getLocale(),
                    context.getMethod(),context.getVersion(),
                    SystemParameterNames.getAccessToken());
        }
        //2.检查会话
        mainError = checkSession(context);
        if (mainError != null) {
        	logger.info("客户端{}:会话验证为不合法",context.getAppKey());
            return mainError;
        }

        //3.检查method参数
        if (context.getMethod() == null) {
        	logger.info("客户端{}:method参数验证为不合法",context.getAppKey());
        ...
        if (context.getVersion() == null) {
        	logger.info("客户端{}:Version参数验证为不合法",context.getAppKey());
        ...
        if (mainError != null) {
        	logger.info("客户端{}:签名不正确",context.getAppKey());
            return mainError;
        }

        //6.检查服务方法的版本是否已经过期
        if (context.getServiceMethodDefinition().isObsoleted()) {
        	logger.info("客户端{}:服务方法的版本已经过期",context.getAppKey());
        ...
        if (mainError != null) {
        	logger.info("客户端{}:请求HTTP方法不匹配",context.getAppKey());
            return mainError;
        }

        //8.检查 format
        if (!MessageFormat.isValidFormat(context.getFormat())) {
        	logger.info("客户端{}:format不对",context.getAppKey());
    ...
    public MainError validateOther(RopRequestContext rrctx) {
        ...
        if (mainError != null) {
        	logger.info("客户端{}:应用/用户没有权访问目标服务",rrctx.getAppKey());
            return mainError;
        }
        
        //2.判断accessToken是否合法
        mainError = checkAccessToken(rrctx);
        if(mainError != null){
        	//logger.info("{}:accessToken不合法",rrctx.getAppKey());
        	return mainError;
        }
        
        //3.判断应用/会话/用户访问服务的次数或频度是否超限
        mainError = checkInvokeTimesLimit(rrctx);
        if (mainError != null) {
        	logger.info("客户端{}:应用/会话/用户访问服务的次数或频度超限",rrctx.getAppKey());
            return mainError;
        }

        //4.如果是上传文件的服务，检查文件类型和大小是否满足要求
        mainError = checkUploadFile(rrctx);
        if (mainError != null) {
        	logger.info("客户端{}:上传文件类型和大小不满足要求",rrctx.getAppKey());
            return mainError;
        }
        ...
        if (mainError != null) {
        	logger.info("客户端{}:业务参数不合法",rrctx.getAppKey());
            return mainError;
        }
	...
	public void setOauthAccessTokenController(OauthAccessTokenController oauthAccessTokenController) {
		this.oauthAccessTokenController = oauthAccessTokenController;
	}
    ...
    public OauthAccessTokenController getOAuthAccessTokenController() {
    	return oauthAccessTokenController;
    }
    ...
    private MainError checkServiceAccessAllow(RopRequestContext smc) {
        if (!getServiceAccessController().isAppGranted(smc.getAppKey(), smc.getMethod(), smc.getVersion())) {
            ...
            if (mainError != null) {
                logger.info("未向ISV开放该服务的执行权限({})",smc.getMethod());
            }
            return mainError;
        } else {
            if (!getServiceAccessController().isUserGranted(smc.getSession(), smc.getMethod(), smc.getVersion())) {
                ...
                mainError.addSubError(subError);
                if (mainError != null) {
                    logger.info("未向会话用户开放该服务的执行权限({})",smc.getMethod());
                }
                return mainError;
            }
            return null;
    ...
    private MainError checkAccessToken(RopRequestContext smc) {
    	if(!getOAuthAccessTokenController().isValid(smc)){
    		MainError mainError = MainErrors.getError(
                    MainErrorType.INVALID_ACCESS_TOKEN, smc.getLocale(),
                    smc.getMethod(), smc.getVersion());
            SubError subError = SubErrors.getSubError(SubErrors.getSubErrorCode(SubErrorType.ISV_INVALID_PARAMETE, SystemParameterNames.getAccessToken()),
                    SubErrorType.ISV_INVALID_PARAMETE.value(),
                    smc.getLocale(),
                    smc.getAccessToken());
            mainError.addSubError(subError);
            if (mainError != null) {
                logger.info("客户端{}:访问令牌无效",smc.getAppKey());
            }
            return mainError;
    	}
    	else{
    		if(!getOAuthAccessTokenController().isAccess(smc)){
				MainError mainError = SubErrors.getMainError(SubErrorType.ISV_INVALID_PERMISSION, smc.getLocale());
				SubError subError = SubErrors.getSubError(SubErrorType.ISV_INVALID_PERMISSION.value(),
						SubErrorType.ISV_INVALID_PERMISSION.value(),
						smc.getLocale());
				mainError.addSubError(subError);
				if (mainError != null) {
					logger.info("未向ISV开放该服务的执行权限({}-{})", smc.getMethod(),smc.getVersion());
				}
				return mainError;
    		}
    	}
            ...
            if (!context.getServiceMethodDefinition().isIgnoreSign()) {
                if (context.getSign() == null) {
                    ...
                    if (!signValue.equals(context.getSign())) {
                        logger.info("客户端{}:签名不合法，请检查",context.getAppKey());
                        return MainErrors.getError(
                ...
                logger.warn("{}忽略了签名",context.getMethod());
                return null;
            }
        } else {
    ...
    private boolean isValidSession(RopRequestContext smc) {
        if (sessionManager.getSession(smc.getSessionId()) == null) {
            logger.info("{}会话不存在，请检查。",smc.getSessionId());
            return false;
        } else {
            return true;
        }
